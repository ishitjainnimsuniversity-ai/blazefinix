...
export async function fetchSamplePatientsList(): Promise<any[]> {
  const res = await fetch(`${API_BASE}/reports/sample-patients`);
  if (!res.ok) {
    throw new Error(`Failed to load sample patients (HTTP ${res.status}).`);
  }
  return await res.json();
}
...
export async function uploadPatientReportPdfApi(
  ...
  formData.append('num_qubits', numQubits.toString());
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => '');
    throw new Error(
      `LOCAL COMPUTATION OFFLINE: Patient PDF parsing failed (HTTP ${res.status}): ${errBody}`
    );
  }
...
export async function evaluateQmlCmlApi(
  features20q: any[],
  numQubits: number = 20
): Promise<any> {
  const res = await fetch(`${API_BASE}/reports/evaluate-qml-cml`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => '');
    throw new Error(
      `LOCAL COMPUTATION OFFLINE: Dual QML/CML evaluation failed (HTTP ${res.status}): ${errBody}`
    );
  }
...
export async function downloadQmlCmlPdfApi(payload: any): Promise<Blob> {
  const res = await fetch(`${API_BASE}/reports/download-qml-cml-pdf`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  });
